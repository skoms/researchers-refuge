...
export const postHandlers = [
  ...
  rest.post(getUrl('/admin/users'), (req, res, ctx) => {
    ...
    if (firstName && lastName && emailAddress && accessLevel && password) {
      return res(
        ctx.status(201),
        ctx.json(getUpdatedData(getMockUsers, req.body)),
      )
    } else {
      return res(
        ctx.status(400),
  ...
  rest.post(getUrl('/admin/topics'), (req, res, ctx) => {
    ...
    if (name && categoryId && relatedTags) {
      return res(
        ctx.status(201),
        ctx.json(getUpdatedData(getMockTopics, req.body)),
      )
    } else {
      return res(
        ctx.status(400),
  ...
  rest.post(getUrl('/admin/categories'), (req, res, ctx) => {
    ...
    if (name) {
      return res(
        ctx.status(201),
        ctx.json(getUpdatedData(getMockCategories, req.body)),
      )
    } else {
      return res(
        ctx.status(400),
  ...
  rest.post(getUrl('/admin/reports'), (req, res, ctx) => {
    ...
    if (title && description && userId) {
      return res(
        ctx.status(201),
        ctx.json(getUpdatedData(getMockReports, req.body)),
      )
    } else {
      return res(
        ctx.status(400),
