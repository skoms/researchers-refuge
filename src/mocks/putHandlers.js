  ...
  rest.put(getUrl('/admin/users'), (req, res, ctx) => {
    ...
    if ( firstName && lastName && emailAddress && accessLevel && password ) {
      ...
      return res(
        ctx.status(201),
        ctx.json(
          getUpdatedData(getMockUsers, req.body, id)
        )
      )
    } else {
      return res( 
  ...
  rest.put(getUrl('/admin/articles'), (req, res, ctx) => {
    ...
    if ( title && topic && intro && body && tags && authorId && published ) { 
      ...
      return res(
        ctx.status(201),
        ctx.json(
          getUpdatedData(getMockArticles, req.body, id)
        )
      )
    } else {
      return res( 
  ...
  rest.put(getUrl('/admin/topics'), (req, res, ctx) => {
    ...
    if ( name && categoryId && relatedTags ) {
      ...
      return res(
        ctx.status(201),
        ctx.json(
          getUpdatedData(getMockTopics, req.body, id)
        )
      )
    } else {
      return res( 
  ...
  rest.put(getUrl('/admin/categories'), (req, res, ctx) => {
    ...
    if ( name ) {
      ...
      return res(
        ctx.status(201),
        ctx.json(
          getUpdatedData(getMockCategories, req.body, id)
        )
      )
    } else {
      return res( 
  ...
  rest.put(getUrl('/admin/reports'), (req, res, ctx) => {
    ...
    if ( title && description && userId ) {
      ...
      return res(
        ctx.status(201),
        ctx.json(
          getUpdatedData(getMockReports, req.body, id)
        )
      )
    } else {
      return res( 
  ...
  rest.put(getUrl('/articles/credit'), (req, res, ctx) => {
    ...
    mockArticle.credits = ( 
      req.body.credit === 'accredit' ? 
        mockArticle.credits + 1 : mockArticle.credits - 1 
    );

    const mockOwner = getMockUsers({ id: mockArticle.userId });
    mockOwner.credits = ( 
      req.body.credit === 'accredit' ? 
        mockOwner.credits + 1 : mockOwner.credits - 1 
    );

    const mockUser = getMockUsers();
    if ( req.body.credit === 'accredit' ) {
      mockUser.discreditedArticles = mockUser.discreditedArticles.filter( entry => entry !== id );
      !mockUser.accreditedArticles.includes(id) && mockUser.accreditedArticles.push(id);
    } else {
      mockUser.accreditedArticles = mockUser.accreditedArticles.filter( entry => entry !== id );
      !mockUser.discreditedArticles.includes(id) && mockUser.discreditedArticles.push(id);
    }
