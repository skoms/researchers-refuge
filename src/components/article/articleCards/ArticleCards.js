...
const ArticleCards = (props) => {
  ...
  useEffect(() => {
    const getRecentlyAccredited = async () => {
      const slicedAndSorted = [...props.recentlyAccredited]
        .reverse()
        .slice(0, 5)
      if (slicedAndSorted.length > 0) {
        await Promise.all(
          slicedAndSorted.map(async (id) => {
            return await dispatch(getArticleInfo(id)).then(
              (res) => res.payload.article,
            )
          }),
  ...
  const articlesOrEmptyMessage = (articles) =>
    articles.length !== 0 ? (
      ...
      <div className={styles.noArticlesMessage}>
        <img
          src={getIconUrl('shrug', darkModeOn, {
            size: 50,
  ...
  if (props.type !== 'accreditedArticles') {
    return (
      <div className={styles.container} data-testid="article-cards-component">
        {didLoad && articles ? articlesOrEmptyMessage(articles) : <Loading />}
        {articles && articles.length !== 0 && lastPage !== 1 && (
          <PaginationBar />
        )}
      </div>
    )
    ...
    return (
      <div className={styles.container} data-testid="article-cards-component">
        {didLoad && secondaryArticles ? (
          articlesOrEmptyMessage(secondaryArticles)
        ) : (
          <Loading />
        )}
        {secondaryArticles &&
          secondaryArticles.length !== 0 &&
          lastPage !== 1 && <PaginationBar />}
      </div>
    )
  }
}
