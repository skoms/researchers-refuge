...
describe('EditProfile', () => {
  ...
  describe('General / Profile', () => {
    ...
    it('should render with the users data already filled in', () => {
      ...
      expect(
        getByLabelText(/occupation/i)
      ).toHaveDisplayValue(mockUser.occupation);
      expect(
        getByLabelText(/bio/i)
      ).toHaveDisplayValue(mockUser.bio);
    });

    it('should update the inputs on user input', () => {
      const { getByLabelText, getAllByDisplayValue } = renderComponent(EditProfile, options);
      userEvent.type(
        getByLabelText(/first name/i),
        'test'
      );
      ...
      expect(
        getAllByDisplayValue(/test$/i)
      ).toHaveLength(4);
    });
    
    it('should successfully update any of the fields that were changed', async () => {
      const { store, getByLabelText, getByRole } = renderComponent(EditProfile, options);
      ...
      await waitFor(() => {
        ...
        expect(
          store.getState().userAcc.authenticatedUser.firstName
        ).toBe('test');
        expect(
          store.getState().userAcc.authenticatedUser.lastName
        ).toBe('test');
      })
    });
    ...
    it('should forward user to /forbidden if error code is not 500', async () => {
      server.use(
        rest.put('*', (req, res, ctx) => res( ctx.status(401) ))
      );
      const { getByLabelText, getByRole } = renderComponent(EditProfile, options);
      userEvent.clear(getByLabelText(/first name/i));
      userEvent.type( getByLabelText(/first name/i), 'test' );
      userEvent.clear(getByLabelText(/last name/i));
      userEvent.type( getByLabelText(/last name/i), 'test' );
  ...
  describe('Account', () => {
    ...
    it('should render with the users data already filled in', () => {
      ...
      ).toHaveDisplayValue(mockUser.emailAddress);
    });
    ...
    it('should update the fields on render update', () => {
      ...
      ]).toStrictEqual(['testCurPass', 'testEmail', 'testNewPass', 'testConPass']);
    });
    ...
    it('should update the info on correct current password', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          store.getState().userAcc.authenticatedUser.emailAddress
        ).toBe('newTest@Email.com');
      })
    });
    ...
    it('should not update the info on wrong current password', async () => {
      ...
      userEvent.type( getByLabelText(/current password/i), mockUser.password + 'wrong' );
      ...
      await waitFor(() => {
        expect(
          getByLabelText(/current password/i).parentElement
