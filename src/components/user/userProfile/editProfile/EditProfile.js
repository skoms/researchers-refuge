  ...
  const submit = async (e) => {
    ...
    if (form === 'Profile') {
      if (
        ...
        validateField('name', firstName, '#firstName-input-div')
      )
        updatedData.firstName = firstName

      if (
        user.lastName !== lastName &&
        validateField('name', lastName, '#lastName-input-div')
      )
        updatedData.lastName = lastName

      if (
        user.occupation !== occupation &&
        validateField('occupation', occupation, '#occupation-input-div')
      )
      ...
      if (oldPassword === user.password) {
        if (
          ...
          validateField('email', email, '#email-input-div')
        )
          updatedData.emailAddress = email
    ...
    Object.keys(updatedData).length > 0 &&
      (await data
        .updateUser(form, user.id, updatedData, user)
        .then((res) => {
          if (res.status === 204) {
          ...
          >
            <img
              src={getIconUrl('x', darkModeOn, {
                size: 16,
                colors: { dark: 'E8F7FF', light: '1A3861' },
              })}
              alt="Exit button"
            />
          </button>
        ...
        <div className={styles.editMain}>
          {form === 'Profile' && (
            <form className={styles.editUserForm} onSubmit={submit}>
              ...
              <div
                className={`form-input ${styles.firstName}`}
                id="firstName-input-div"
              >
                <input
                  id="firstName"
                  name="firstName"
              ...
              <div
                className={`form-input ${styles.lastName}`}
                id="lastName-input-div"
              >
                <input
                  id="lastName"
                  name="lastName"
              ...
              <div
                className={`form-input ${styles.occupation}`}
                id="occupation-input-div"
              >
                <input
                  id="occupation"
                  name="occupation"
              ...
              <div
                className={`form-input ${styles.mostActiveField}`}
                id="mostActiveField-input-div"
              >
          ...
          {form === 'Account' && (
            <form className={styles.editAccountForm} onSubmit={submit}>
              ...
              <div
                className={`form-input ${styles.oldPass}`}
                id="oldPass-input-div"
              >
                <input
                  id="oldPass"
                  name="oldPass"
                ...
                <label htmlFor="oldPass">
                  Current Password <span> *Required to update login info</span>
                </label>
              </div>
              <div
                className={`form-input ${styles.email}`}
                id="email-input-div"
              >
                <input
                  id="email"
              ...
              <div
                className={`form-input ${styles.confPass}`}
                id="confPass-input-div"
              >
                <input
                  id="confPass"
                  name="confPass"
