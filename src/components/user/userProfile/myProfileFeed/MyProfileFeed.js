...
const MyProfileFeed = () => {
  ...
  useEffect(() => {
    const getOwnersArticles = async (id) => {
      await dispatch(getUserArticles({ id, page })).catch((err) => {
        console.error(err)
  ...
  const toggleHeaderUploader = (e = null) => {
    if (
      (e !== null &&
        (e.target.getAttribute('data-value') === 'upload-popup' ||
          e.target.getAttribute('data-value') === 'header-img')) ||
      e === null
    ) {
      const headerImgPopup = document.querySelector(
        '.invisibility-container-header',
      )
  ...
  const toggleProfileUploader = (e = null) => {
    if (
      (e !== null &&
        (e.target.getAttribute('data-value') === 'upload-popup' ||
          e.target.getAttribute('data-value') === 'profile-pic')) ||
      e === null
    ) {
      const profilePicPopup = document.querySelector(
        '.invisibility-container-profile',
      )
  ...
  return owner !== null ? (
    <div className={styles.container}>
      ...
      <div className="invisibility-container-header invisible">
        <ImageUploader
          toggleHeaderUploader={toggleHeaderUploader}
          purpose="header"
        />
      </div>
      <div className="invisibility-container-profile invisible">
        <ImageUploader
          toggleProfileUploader={toggleProfileUploader}
          purpose="profile"
        />
      </div>
      ...
      <div className={styles.infoHeader}>
        <div className={styles.headerImgDiv}>
          <img
            src={owner.headerImgURL || 'https://placeimg.com/1000/150/tech'}
            alt="header"
            className={styles.headerImg}
            data-value="header"
            onClick={toggleHeaderUploader}
          />
          <img
            src={
              owner.profileImgURL ||
            ...
            alt="profile-pic"
            data-value="profile-pic"
            className={`${styles.profilePic} ${
              owner.profileImgURL ? '' : 'placeholder'
            }`}
        ...
        <div className={styles.nameOccupationAndBio}>
          ...
          <p className={styles.occupation}>
            {capitalize(owner.occupation) || ''}
          </p>
          <ReactMarkdown className={styles.bio}>
            {owner.bio || ''}
          </ReactMarkdown>
        </div>
        <div className={styles.stats}>
          <div className={`${styles.stat} most-active-field`}>
            ...
            <p className={styles.data}>
              {capitalize(owner.mostActiveField) || 'None'}
            </p>
          </div>
          ...
          <div className={`${styles.stat} followers`}>
            ...
            <p className={styles.data}>{owner.followers.length || 0}</p>
          </div>
          <div className={`${styles.stat} following`}>
            <p className={styles.title}>Following:</p>
            <p className={styles.data}>{owner.following.length || 0}</p>
          </div>
        </div>
      </div>
      ...
      <div className={styles.articlesAccreditedContainer}>
        ...
        <ArticleCards
          ...
          recentlyAccredited={owner.accreditedArticles}
        />
      </div>
    </div>
  ) : (
