...
const ImageUploader = ({
  ...
  const uploadImage = async () => {
    ...
    shaSignature.update(
      `public_id=${publicId}&timestamp=${timestamp}&upload_preset=${upload_preset}${dictionary.SECRET}`,
    )
    ...
    if (response) {
      ...
      await data
        .updateUser(`img/${purpose}`, user.id, updatedData, user)
        .then((res) => {
          if (res.status === 204) {
  ...
  return (
    <div
      ...
      onClick={
        purpose === 'header' ? toggleHeaderUploader : toggleProfileUploader
      }
    >
      <div className={styles.uploader}>
        <h2 className={styles.title}>
          {purpose === 'header' ? 'Header Image' : 'Profile Image'}
        </h2>
        <input
          type="file"
          name="file-input"
