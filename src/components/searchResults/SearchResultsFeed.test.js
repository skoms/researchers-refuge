...
describe('SearchResultsFeed', () => {
  it('should render without any errors', () => {
    ...
    expect(getByText(/search results for: 'test'/i)).toBeInTheDocument()
  })
  ...
  describe('with data results', () => {
    it('should render with data for users', async () => {
      ...
      await waitFor(() => {
        expect(getByText(/test2 user2/i)).toBeInTheDocument()
        expect(store.getState().searchResults.users).toStrictEqual(
          getMockUsers({ amount: 6 }).slice(1),
        )
      })
    })

    it('should show next user on click', async () => {
      const { getByRole, getByText, getByAltText } = renderComponent(
        SearchResultsFeed,
        options,
      )
      ...
      await waitFor(() => {
        expect(getByText(/tester3/i)).toBeVisible()
      })
    })
    ...
    it('should show first user after clicking back and forth', async () => {
      ...
      await waitFor(() => {
        expect(getByText(/tester2/i)).toBeVisible()
      })
    })
    ...
    it('should render with data for topics', async () => {
      ...
      await waitFor(() => {
        expect(getByText(/test category1/i)).toBeInTheDocument()
        expect(getByText(/test topic1/i)).toBeInTheDocument()
        expect(store.getState().searchResults.topics).toStrictEqual(
          getMockTopics({ amount: 5 }),
        )
      })
    })
    ...
    it('should render with data for articles', async () => {
      ...
      await waitFor(() => {
        expect(getByText(/^test title1$/i)).toBeInTheDocument()
        expect(store.getState().searchResults.articles).toStrictEqual(
          getMockArticles({ amount: 15 }),
        )
      })
    })
  })
  ...
  describe('without data results', () => {
    it('should render no results message for users', async () => {
      server.use(
        rest.get('*/users/query', (req, res, ctx) =>
      ...
      await waitFor(() => {
        expect(getByText(/no users match your search.../i)).toBeInTheDocument()
        expect(store.getState().searchResults.users).toStrictEqual([])
      })
    })
    ...
    it('should render no results message for topics', async () => {
      server.use(
        rest.get('*/topics/query', (req, res, ctx) =>
          res(ctx.status(200), ctx.json([])),
        ),
      )
      ...
      await waitFor(() => {
        expect(getByText(/no topics match your search.../i)).toBeInTheDocument()
        expect(store.getState().searchResults.topics).toStrictEqual([])
      })
    })
    ...
    it('should render no results message for articles', async () => {
      server.use(
        rest.get('*/articles/query', (req, res, ctx) =>
