...
import {
  selectSearchTerm,
  updateSearchTerm,
} from '../searchField/searchFieldSlice'
import {
  getResults,
  selectArticlesResults,
  selectTopicsResults,
  selectUsersResults,
      ...
      {users && filteredUsers() && filteredUsers().length > 0 ? (
        <div className={`${styles.results} ${styles.users}`}>
          ...
          >
            {!isMobile && (
              <button
                className={`${styles.prev} ${styles.navBtn} ${
                  displayedUser === 0 ? 'invisible' : ''
                }`}
                onClick={(e) => nextPrevButton(e, 'prev')}
              >
                <img
                  src={getIconUrl('prev', darkModeOn)}
                  alt="previous button"
                />
              </button>
            )}
            <div
              key={filteredUsers()[displayedUser].id}
              className={styles.userCard}
            >
              <ResultRecUser
                key={displayedUser}
                type="res"
                user={filteredUsers()[displayedUser]}
              />
            </div>
            {!isMobile && (
              <button
                className={`${styles.next} ${styles.navBtn} ${
                  displayedUser === filteredUsers().length - 1
                    ? 'invisible'
                    : ''
                }`}
                onClick={(e) => nextPrevButton(e, 'next')}
              >
          ...
          <div className={styles.points}>
            {filteredUsers().length > 1 ? (
              filteredUsers().map((user, id) => (
                <div
                  key={id}
                  className={`${styles.point} ${id} ${
                    id === displayedUser ? 'point-selected' : ''
                  }`}
                  onClick={() => setDisplayedUser(id)}
                />
              ))
            ) : (
              <Fragment />
        ...
        <div className={`${styles.results} ${styles.topics}`}>
          ...
          {categories.map((category) => {
            const categoryTopics = topics.filter(
              (topic) => category.id === topic.categoryId,
            )
            return (
