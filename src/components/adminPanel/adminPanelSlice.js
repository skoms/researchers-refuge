...
const initialState = {
  ...
  categories: {
    ...
    requiredColumns: [{ column: 'name', name: 'Name' }],
    newData: {},
    entries: [],
    total: 0,
    rangeStart: 0,
...
export const getUsersAdmin = createAsyncThunk(
  ...
  async ({
    ...
    const response = await data.getUsersAdmin(
      user,
      limit,
      page,
      sortColumn,
...
export const getUsersByQueryAdmin = createAsyncThunk(
  ...
  async ({
    ...
    const response = await data.getUsersByQueryAdmin(
      user,
      query,
      limit,
      page,
...
export const getArticlesAdmin = createAsyncThunk(
  ...
  async ({
    ...
    const response = await data.getArticlesAdmin(
      user,
      limit,
      page,
      sortColumn,
...
export const getArticlesByQueryAdmin = createAsyncThunk(
  ...
  async ({
    ...
    const response = await data.getArticlesByQueryAdmin(
      user,
      query,
      limit,
      page,
...
export const getTopicsAdmin = createAsyncThunk(
  ...
  async ({
    ...
    const response = await data.getTopicsAdmin(
      user,
      limit,
      page,
      sortColumn,
...
export const getTopicsByQueryAdmin = createAsyncThunk(
  ...
  async ({
    ...
    const response = await data.getTopicsByQueryAdmin(
      user,
      query,
      limit,
      page,
...
export const getCategoriesAdmin = createAsyncThunk(
  ...
  async ({
    ...
    const response = await data.getCategoriesAdmin(
      user,
      limit,
      page,
      sortColumn,
...
export const getCategoriesByQueryAdmin = createAsyncThunk(
  ...
  async ({
    ...
    const response = await data.getCategoriesByQueryAdmin(
      user,
      query,
      limit,
      page,
...
export const getReportsAdmin = createAsyncThunk(
  ...
  async ({
    ...
    const response = await data.getReportsAdmin(
      user,
      status,
      limit,
      page,
...
export const getReportsByQueryAdmin = createAsyncThunk(
  ...
  async ({
    ...
    const response = await data.getReportsByQueryAdmin(
      user,
      status,
      query,
      limit,
...
export const adminPanelSlice = createSlice({
  ...
  reducers: {
    ...
    updateSortOrder: (state, { payload }) => {
      const results = state
      if (!payload)
        return { ...state, sortOrder: { column: 'id', order: 'ASC' } }
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(updateEntryAdmin.fulfilled, (state, { payload }) => {
      ...
      let result = state
      if (status === 201) {
        const updatedIndex = result[type].entries.indexOf(
          result[type].entries.find((entry) => entry.id === id),
        )
    ...
    builder.addCase(markReportAsAdmin.fulfilled, (state, { payload }) => {
      ...
      let result = state
      if (status === 204) {
        result.reports.entries = result.reports.entries.filter(
          (entry) => entry.id !== id,
        )
      }
      return result
    ...
    builder.addCase(deleteEntryAdmin.fulfilled, (state, { payload }) => {
      ...
      let result = state
      if (status === 204) {
        result[type].entries = result[type].entries.filter(
          (entry) => entry.id !== id,
        )
        result[type].total--
        result[type].rangeEnd--
