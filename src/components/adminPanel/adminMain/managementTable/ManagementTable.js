...
const ManagementTable = (
  ...
  const formatEntryData = (entry, column, i) => {
    switch (column.column) {
      case 'createdAt':
        return <td className={styles.tableData} key={i}>{`${entry[column.column].slice(0, 10)} ${entry[column.column].slice(11, 16)}`}</td>
      case 'updatedAt':
        return <td className={styles.tableData} key={i}>{`${entry[column.column].slice(0, 10)} ${entry[column.column].slice(11, 16)}`}</td>
      case 'userId':
        return <td className={styles.tableData} key={i}>{`(${entry[column.column]}) ${entry.User ? entry.User.firstName : ''} ${entry.User ? entry.User.lastName : ''}`}</td>
      case 'categoryId':
  ...
  return (
    <>
      ...
      <table className={`${styles.table} ${ statusFilter !== null ? statusFilter : '' }`}>
          <tbody>
            <tr>
              { data.columns.map(column => 
                <th className={styles.tableHeader} data-value={column.column} key={column.column} onClick={handleSort}>
            ...
            { page === 1 &&
              <tr>
                { data.columns.map(column => {
                  if (!column.input) return <td className={styles.tableData} key={column.column}></td>;
                  return <td className={styles.tableData} key={column.column}><input type="text" data-column={column.column} placeholder={column.name} onChange={inputChangeHandler} className={styles.tableInput}/></td>;
                })}
