...
const ManagementTable = ({ data, statusFilter = null }) => {
  ...
  const formatEntryData = (entry, column, i) => {
    switch (column.column) {
      case 'createdAt':
        return (
          <td className={styles.tableData} key={i}>
            {`${entry[column.column].slice(0, 10)} ${entry[column.column].slice(
              11,
              16,
            )}`}
          </td>
      ...
      case 'userId':
        return (
          <td className={styles.tableData} key={i}>
            {`(${entry[column.column]}) ${
              entry.User ? entry.User.firstName : ''
            } ${entry.User ? entry.User.lastName : ''}`}
          </td>
        )
      case 'categoryId':
        return (
          ...
          {page === 1 && (
            <tr>
              {data.columns.map((column) => {
                if (!column.input)
                  return (
                    <td className={styles.tableData} key={column.column}></td>
                  )
                return (
                  <td className={styles.tableData} key={column.column}>
                    <input
                      type="text"
