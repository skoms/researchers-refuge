...
describe('StatusFilter', () => {
  beforeEach(() => {
    renderComponent(StatusFilter, {
      expectedProps: { setStatusFilter: mockSetStatusFilter },
    })
  })
  ...
  it('should render without any errors', () => {
    expect(
      screen.getAllByRole('button', { name: /(open)|(resolved)|(rejected)/i })
        .length,
    ).toBe(3)
  })
  ...
  it('should remove "selected" from previous option when selecting new option', () => {
    ...
    expect(screen.getByRole('button', { name: /open/i })).not.toHaveClass(
      'selected',
    )
  })
  ...
  it('should add "selected" to the new option', () => {
    userEvent.click(screen.getByRole('button', { name: /rejected/i }))
    expect(screen.getByRole('button', { name: /rejected/i })).toHaveClass(
      'selected',
    )
  })
})
