...
const DataManager = ({ setManagerProps, isActive, source, data, type }) => {
  ...
  const submit = async (e) => {
    ...
    if (source === 'edit') {
      ...
      dispatch(
        await updateEntryAdmin({ user, type, id: data.id, body: updatedData }),
      )
  ...
  return (
    <div
      ...
      data-testid="data-manager-component"
    >
      {source === 'view' ? (
        <>
              ...
              {Object.keys(data).map((key) => (
                <tr key={type + key}>
                  {typeof data[key] !== 'object' ? (
                    ...
                    <th className={styles.tableHead}>{key + ' (id)'}</th>
                  )}
                  {typeof data[key] !== 'object' ? (
          ...
          <div className={styles.buttons}>
            <button className={styles.buttonSecondary} onClick={closePopup}>
              Close
            </button>
          </div>
        </>
