...
describe('MenuBar', () => {
  ...
  describe('general', () => {
    ...
    it('should render all 4 filter buttons and 1 "New Article" button', () => {
      expect(
        screen.getAllByRole('button', { name: /(popular)|(new)|(top)|(following)$/i })
      ).toHaveLength(5);
    });
    ...
    it('should render "new article" button and not icon', () => {
      expect(
        screen.queryByAltText(/write article button/i)
      ).toBeNull();
      expect(
  ...
  describe('on mobile', () => {
    ...
    beforeEach(() => {
      act(() => { renderComponent(MenuBar, { needsStore, preloadedState }) })
    });

    it('should render icon instead of text for "new article" button', () => {
      expect(
        screen.getByAltText(/write article button/i)
      ).toBeInTheDocument();
      expect(
        screen.queryByRole('button', { name: /new article/i })
      ).toBeNull();
    });
  });
});
