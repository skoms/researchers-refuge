...
describe('DarkModeButton', () => {
  ...
  it('should initially render with moon icon', () => {
    expect(
      screen.getByAltText(/darkmode button/i).src
    ).toBe('https://img.icons8.com/ios-filled/24/FFFFFF/crescent-moon.png')
  });
  ...
  it('should render sun icon after clicked', () => {
    ...
    expect(
      screen.getByAltText(/darkmode button/i).src
    ).toBe('https://img.icons8.com/material-rounded/24/FFFFFF/sun--v1.png')
  });

  it('should toggle the darkmode after clicked', () => {
    userEvent.click(screen.getByAltText(/darkmode button/i));
    expect(
      store.getState().darkModeButton.darkModeOn
    ).toBe(true);
  });
