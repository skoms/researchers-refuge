...
const ResultRecUser = ({ user }) => {
  ...
  const followUnfollow = async (e) => {
    ...
    const response = await dispatch(
      followUser({ id: user.id, user: authenticatedUser }),
    )
  ...
  return (
    <div key={user.id} className={styles.container}>
      ...
      <a href={`/users/${user.id}`} className={styles.fullName}>
        <p>{capitalize(`${user.firstName} ${user.lastName}`, false)}</p>
      </a>
      ...
      <div className={styles.buttons}>
        ...
        />
        <TypedButton buttontype="secondary" content="View" onClick={view} />
      </div>
    </div>
  )
}
